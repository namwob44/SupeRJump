...
#' Wrapper Function for RNA expression within a single cell group for bias vs non bias cells
#'
#' @param suerat_obj 
#' @param poise_df 
...
#'
#' @examples
RunDifferentialExpressionOnPoiseVsNonPoise<-function(suerat_obj,poise_df,lineages_to_compare,foldername){
  ...
  marker_object<-Seurat::FindMarkers(object =suerat_obj@assays[["tfsulm"]],cells.1=cells_group_1,cells.2=cells_group_2)
  
  write.csv(marker_object,file = paste0(foldername,"TF_",as.character(poise_df$Cluster.Assignment)[1],"_poising_for_",lineages_to_compare,".csv"),sep = ",",append = T)
  return(TRUE)
}
