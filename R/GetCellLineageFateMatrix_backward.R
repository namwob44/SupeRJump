#' Getting the Fates and Lienages in reverse strategy
#'
#' @param Bmat 
#' @param sink_cells 
#'
...
#'
#' @examples
GetCellLineageFateMatrix_backward<-function(Bmat,sink_cells){
  ...
  for(sink_iter in 1:length(sink_groups)){
    ...
    if(length(lineage_columns)>1){
      Bmat_condensed[,sink_iter]<-rowProds(Bmat[,colnames(Bmat)%in%lineage_columns],na.rm=T)
    }else{
