#' Volcano plots comparing a poised and nonpoised for lineages of a specific cell type
#'
#' @param reduced_data_frame 
#' @param lineages_we_care_about 
#' @param folder_name 
#'
#' @return
#' @export
#'
#' @examples
GetDiffBetweenPoisedAndNonpoisedCells<-function(reduced_data_frame,lineages_we_care_about,folder_name){
  if(length(as.character(unique(reduced_data_frame$Poised)))<2){
    return(NULL)
  }
  ...
  p_values_TF_table<-reduced_data_frame%>%
    tidyr::pivot_longer(cols = !c(Poised,Group,Combined_Ordering,Cluster.Assignment),
                        ...
                        values_to = "Score")%>%
    group_by(TF_names)%>%
    reframe(ttest =list(pairwise.wilcox.test(x=Score,g=Poised,p.adjust.method="bonf",pool.sd = F))) %>%
  ...
  mean_groups_TF_table<-reduced_data_frame%>%
    tidyr::pivot_longer(cols = !c(Poised,Group,Combined_Ordering,Cluster.Assignment),
                        names_to = "TF_names",
                        values_to = "Score")%>%
    group_by(TF_names,Poised)%>%
    summarise(across(Score,mean))%>%
    mutate(log_score = log2(Score))
  
  Log2FC_pvalue_df<-data.frame(TF_names=mean_groups_TF_table%>%dplyr::filter(Poised==as.character(p_values_TF_table$group1[1]))%>%pull(TF_names),
                               log2FC=(mean_groups_TF_table%>%dplyr::filter(Poised==as.character(p_values_TF_table$group1[1]))%>%pull(Score))-(mean_groups_TF_table%>%dplyr::filter(Poised==as.character(p_values_TF_table$group2[1]))%>%pull(Score)))%>%
  ...
  ggsave(filename = paste0(folder_name,"_",lineages_we_care_about,"_",as.character(Log2FC_pvalue_df$group1[1]),"_vs_",as.character(Log2FC_pvalue_df$group2[1]),".pdf"),width=9,height=6,plot=last_plot())
